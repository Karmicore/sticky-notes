...
/// Sync helper for tray menu — copy export
pub fn do_export_copy(ids: &[i32], app: &tauri::AppHandle, svc: &NoteService) -> Result<(), String> {
    let notes = fetch_notes(ids, svc)?;
    let markdown = notes_to_markdown(&notes);
    app.clipboard()
        .write_text(markdown)
...
/// Sync helper for tray menu — cut export
pub fn do_export_cut(ids: &[i32], app: &tauri::AppHandle, svc: &NoteService) -> Result<(), String> {
    let notes = fetch_notes(ids, svc)?;
    let markdown = notes_to_markdown(&notes);
    app.clipboard()
        .write_text(markdown)

...
export default function NoteEditor({ note, update, insertCheckboxRef, style }) {
  ...
  const toggleCb = useCallback((idx) => {
    ...
    if (next === "⏳ ") {
      confetti({ particleCount: 1, spread: 5, startVelocity: 5, gravity: -0.15, ticks: 300, origin: { x: 0.5, y: 1 }, shapes: ["circle"], scalar: 0.2, colors: ["#fff9c4"] });
    }
    if (next === "✅ ") {
  ...
  const handleTextareaClick = useCallback(() => {
    ...
    console.log("[checkbox]", { cursorPos, charInLine, CB_LEN, line: JSON.stringify(line), match: CHECKBOX_RE.test(line) });
    if (!CHECKBOX_RE.test(line)) return;
